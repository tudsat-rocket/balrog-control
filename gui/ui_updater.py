...
from shared.shared_queues import n2o_main_servo_sensor_list, n2_purge_servo_sensor_list
...
def update_valve_states(self):
    if len(n2o_main_servo_sensor_list[1]) > 0:
        state = n2o_main_servo_sensor_list[1][-1]
        self.label_valve_status_n20_main_state.setText(str(state))
    if len(n2_purge_servo_sensor_list[1]) > 0:
        state = n2_purge_servo_sensor_list[1][-1]
